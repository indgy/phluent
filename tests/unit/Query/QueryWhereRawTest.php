...
class QueryWhereRawTest extends \PHPUnit\Framework\TestCase
{
    ...
    // test whereRaw
    // test whereRawAndWhereRaw
    // test whereRawWhereNotRaw
    // test whereRawOrWhereRaw
    // test whereRawOrWhereNotRaw

    public function testWhereRaw()
    {
        $expected = "SELECT * FROM `contacts` WHERE name=?";
        
        $q = new Query();
        $q->table('contacts')->whereRaw('name=?', ['Bob']);
    ...
    {
        $expected = "SELECT * FROM `contacts` WHERE NOT name=?";

        $q = new Query();
        $q->table('contacts')->whereNotRaw('name=?', ['Bob']);

        $this->assertEquals($expected, $q->getSql());
        $this->assertEquals(['Bob'], $q->getParams());
    }
    public function testWhereAndWhereRaw()
    {
        $expected = "SELECT * FROM `contacts` WHERE `contacts`.`name`=? AND email LIKE ?";
        
        $q = new Query();
        $q->table('contacts')->where('name', 'Bob')->whereRaw('email LIKE ?', ['@gmail.com']);

        $this->assertEquals($expected, $q->getSql());
        $this->assertEquals(['Bob', '@gmail.com'], $q->getParams());
    }
    public function testWhereAndWhereNotRaw()
    {
        $expected = "SELECT * FROM `contacts` WHERE `contacts`.`name`=? AND NOT email LIKE ?";
        
        $q = new Query();
        $q->table('contacts')->where('name', 'Bob')->whereNotRaw('email LIKE ?', ['@gmail.com']);

        $this->assertEquals($expected, $q->getSql());
        $this->assertEquals(['Bob', '@gmail.com'], $q->getParams());
    }
    public function testWhereOrWhereRaw()
    {
        $expected = "SELECT * FROM `contacts` WHERE `contacts`.`name`=? OR email LIKE ?";
        
        $q = new Query();
        $q->table('contacts')->where('name', 'Bob')->orWhereRaw('email LIKE ?', ['@gmail.com']);

        $this->assertEquals($expected, $q->getSql());
        $this->assertEquals(['Bob', '@gmail.com'], $q->getParams());
    }
    public function testWhereOrWhereNotRaw()
    {
        $expected = "SELECT * FROM `contacts` WHERE `contacts`.`name`=? OR NOT email LIKE ?";
        
        $q = new Query();
        $q->table('contacts')->where('name', 'Bob')->orWhereNotRaw('email LIKE ?', ['@gmail.com']);
